Feature: Add psc link to company profile

  Scenario Outline: Add psc link successfully

    Given the CHS Kafka API is reachable
    And   the company profile resource "<data_file>" exists for "<company_number>"
    And   the psc link does not exist for "<company_number>"
    When  a PATCH request is sent to the add psc link endpoint for "<company_number>"
    Then  the response code should be 200
    And   the psc link exists for "<company_number>"
  ...
  Scenario Outline: Add psc link unsuccessfully - user not authenticated or authorised
    ...
    When  a PATCH request is sent to the add psc endpoint for "<company_number>" without ERIC headers
